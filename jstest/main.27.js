 ...
 * 26.3.1      Mar-19-26       Kevin Lazarz      Fix quiz badge relative path resolution and guard TOC active-item lookup
 */
...
let main = function () {
    ...
    let navModule = null;
    const navModuleDeferred = $.Deferred();
    const navReadyDeferred = $.Deferred();
    ...
    (function loadNavigationModule() {
        const scriptSource = resolveCurrentMainScriptSource();
        let navModuleUrl = "modules/navigation.js";

        if (scriptSource) {
            navModuleUrl = scriptSource.replace(/main(?:\.[0-9.]+)?(?:\.min)?\.js(?:[?#].*)?$/, "modules/navigation.js");
        }

        const initializeNavModule = function () {
            navModule = window.LiveLabsNav || null;
            navModuleDeferred.resolve(navModule);
        };

        if (window.LiveLabsNav && typeof window.LiveLabsNav.setupTutorialNav === "function") {
            initializeNavModule();
            return;
        }

        $.getScript(navModuleUrl)
            .done(initializeNavModule)
            .fail(function (err) {
    ...
    $(document).ready(function () {
        ...
        $.when(
            ...
            navReadyDeferred.promise()
        ).done(function () {
            init();
            const navApi = navModule;
            let selectedTutorial;
            if (navApi && typeof navApi.setupTutorialNav === "function") {
                selectedTutorial = navApi.setupTutorialNav(manifestFileContent);
            } else {
                selectedTutorial = manifestFileContent.tutorials ? manifestFileContent.tutorials[0] : null;
            }

            let articleElement = document.createElement('article'); //creating an article that would contain MD to HTML converted content

            let toggleNavHandler = function () { };
            if (navApi && typeof navApi.toggleTutorialNav === "function") {
                toggleNavHandler = navApi.toggleTutorialNav;
            }

            loadTutorial(articleElement, selectedTutorial, manifestFileContent, toggleNavHandler);

            if (navApi && typeof navApi.prepareToc === "function") {
                navApi.prepareToc(manifestFileContent);
            }

            if (navApi && typeof navApi.setupRelatedSection === "function") {
                navApi.setupRelatedSection(manifestFileContent);
            }

            if (navApi && typeof navApi.handleInitialNavigationState === "function") {
                navApi.handleInitialNavigationState();
            }
        });
    });
    ...
    let init = function () {
        ...
        $('#openNav').click(function () {
            if (navModule && typeof navModule.handleNavToggleClick === "function") {
                navModule.handleNavToggleClick();
            }
        });
    ...
    const loadTutorial = function (articleElement, selectedTutorial, manifestFileContent, callbackFunc = null) {
        ...
        $.get(tut_fname, function (markdownContent) { //reading MD file in the manifest and storing content in markdownContent variable
            ...
            if (navModule && typeof navModule.showRightAndLeftArrow === "function") {
                articleElement = navModule.showRightAndLeftArrow(articleElement, manifestFileContent);
            }
    ...
    const setupContentNav = function (manifestFileContent, articleElement) {
        ...
        if (navModule && typeof navModule.selectTutorial === "function") {
            currentNavTutorial = navModule.selectTutorial(manifestFileContent);
    ...
    navModuleDeferred.done(function (module) {
        if (module && typeof module.configure === "function") {
            module.configure({
                ...
                singlesource,
                queryParam,
                navParamName: nav_param_name,
                headerParamName: header_param_name,
                createShortNameFromTitle,
                relatedPath: related_path,
                expandSectionBasedOnHash
            });
            if (typeof module.bindScrollHandler === "function") {
                module.bindScrollHandler();
            }
            if (typeof module.bindHashChange === "function") {
                module.bindHashChange();
            }
        }
        navReadyDeferred.resolve(module);
    });
